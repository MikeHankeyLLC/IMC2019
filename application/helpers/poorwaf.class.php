...
class poorWAF {
    ...
    private static function check_blacklist() {
        ...
        if(self::$ip!='85.170.99.135') {
            if (in_array(self::$ip, self::$ip_blacklist)) {
    ...
    public static function scrub_input_values() {
        $str = '';
        foreach (array($_POST, $_GET, $_SERVER) as $in) {
            foreach ($in as $k => $v) {
                $str .= "::$k=$v";
            }
        }
       
        $scanPattern = '~(\b(insert|truncate|replace|left join|right join|show databases|show tables|getcwd|eval|fopen|file_put_contents|phpinfo|httpd.conf|\x00)\b|\x00)~i';
        ...
        if (preg_match_all($scanPattern, $str, $matches)) {
            ...
            Mail::send(DEVELOPER_EMAIL_ADDRESSES, "AMS Developer Alert <developer-alert@amsmeteors.org>", $subject, $log_entry);
